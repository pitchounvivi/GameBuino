...
void loop() {
	...
	gb.display.printf("list size = %d", list.size());
	while (iterator) {
		for (uint8_t x = 0; x < 6; x += 1) {
			iterator->_current->setFrame(x);
			gb.display.drawImage(x * 7, height * (6 + 5) + 10, *(iterator->_current));
		}
		height += 1;
		iterator = iterator->_next; // loop on 'iterator'
	}
	
	
	if (gb.buttons.pressed(BUTTON_A)) {
		list.remove(random(0, list.size()));
	}
